...
Mentre gli altri moduli Monitor sono solo dei sensori che si occupano di fornire dati, il "River Monitor" è un complesso sistema progettato per rilevare, monitorare e rispondere alle situazioni di alluvione in aree sensibili. Questo paragrafo descrive in dettaglio il design concettuale del sistema, delineando i componenti chiave e il loro ruolo all'interno dell'architettura complessiva.
...
\subsubsection{Sensori}\label{sec:Sensori}
...
\subsubsection{Interazioni tra i componenti del River Monitor}
...
\begin{enumerate}
    \item \textbf{Flood Sensor}: Sensore che rileva il livello dell'acqua del fiume, descritto nella sezione \ref{sec:Sensori}.
...
Di seguito viene fornito un esempio di comando per avviare il modulo \textit{Core}:
...
\subsection{River Monitor}
Il modulo monitor \textit{River Monitor} è un sistema per il monitoraggio dei fiumi. In Figura \ref{fig:river-monitor-view} viene mostrata l'interfaccia grafica, la quale mostra i dettagli della zona monitorata e offre la possibilità di intervenire per evacuarla in caso di emergenza.

\begin{figure}[H]
    \caption{Interfaccia grafica del modulo \textit{River Monitor}.}
    \includegraphics[width=\textwidth]{../assets/images/river-monitor-view.png}
    \label{fig:river-monitor-view}
\end{figure}
